...
- [ ] Add HTML container above the chat input box to hold image thumbnails (hidden by default).
- [ ] Add CSS styling for the image thumbnail container (e.g., small preview, remove 'X' button).
- [ ] Implement a `paste` event listener on the `window` or `document` to intercept clipboard items.
- [ ] Implement a function to read the pasted `File` object using `FileReader` as a Base64 Data URL.
- [ ] Display the Base64 Data URL as an `<img>` tag in the thumbnail container.
- [ ] Add functionality to clear/remove the attached image before sending.

## Phase 2: Communication & WebSocket Payload
- [ ] Update the `Enter` keypress logic: when sending a message, check if there is an attached image.
- [ ] Modify the JSON payload sent via WebSocket. Change from `{"message": "text"}` to a structured payload: `{"message": "text", "images": ["data:image/png;base64,..."]}`.
- [ ] Update the `appendUserMessage` function in the UI so that if an image is sent, it is rendered inside the user's chat bubble as an `<img>` tag alongside the text.
- [ ] Clear the thumbnail container and reset state after a successful send.
