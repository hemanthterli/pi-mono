    ...
    switch (name) {
        ...
        case 'edit': {
            ...
            if (!content.includes(old_string)) {
                return `Error: string not found in ${resolved}`;
            }
        ...
        case 'grep': {
            ...
            const searchDir = (dir) => {
                try {
                    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                        if (entry.name.startsWith('.') || ['node_modules', '__pycache__', '.git'].includes(entry.name)) {
                            continue;
                        }
            ...
            try {
                fs.statSync(resolved).isDirectory() ? searchDir(resolved) : searchFile(resolved);
            }
            catch (e) {
            ...
            const listDir = (dir, depth) => {
                try {
                    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                        ...
                        if (entry.isDirectory() && recursive) {
                            listDir(nodePath.join(dir, entry.name), depth + 1);
                        }
                    }
                }
                catch { }
